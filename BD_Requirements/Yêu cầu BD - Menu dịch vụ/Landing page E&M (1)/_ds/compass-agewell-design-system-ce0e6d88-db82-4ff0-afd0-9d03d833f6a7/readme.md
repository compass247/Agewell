...
color and type tokens, the Creato Display webfont, reusable React primitives, foundation
...
**Type.** Single family — **Creato Display** (modern geometric sans). ExtraBold (800) for
...
- `colors.css`, `typography.css`, `spacing.css` (spacing + radius + shadow + motion), `base.css`
  (element defaults + `.eyebrow` helper), `fonts.css` (`@font-face`), `fonts/` (Creato Display OTF).
...
- `@font-face` references the OTF files directly (woff2 not supplied).
